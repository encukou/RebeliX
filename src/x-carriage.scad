...
module x_carriage_base(){
 ...
 // Zakladna
 translate([-part_width/2,-12,0]) cube([part_width,73,7+1.5]);
}
...
module x_carriage_holes(){
  ...
  //
  for(x=[-part_width/4,part_width/4]) {
    translate([x,64,-1])cylinder(r=5,h=20,$fn=30);
    translate([x,64,2])difference() {
        cylinder(r=9,h=4,$fn=30);
        translate([0,0,-1])cylinder(r=7,h=7,$fn=30);
    }
    translate([x,65,-1])cylinder(r=5,h=20,$fn=30);
  }
...
module x_carriage_fancy(){
 // Horni pravy roh
 translate([part_width/2,x_rod_distance+12-2.5,-0.1]) rotate([0,0,30]) cube([30,30,20]);
 // Horni levy roh
 translate([-part_width/2,x_rod_distance+12-2.5,-0.1]) rotate([0,0,60]) cube([30,30,20]);
 // Dolni pravy roh
